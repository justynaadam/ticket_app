...
class Search < ApplicationRecord
  ...
  def find_tickets
    ...
    tickets = tickets.where('tickets.title LIKE ? ', "%#{keywords}%")            if !in_content? && keywords.present?
    tickets = tickets.where('tickets.title LIKE ? OR tickets.content LIKE ? ', "%#{keywords}%", "%#{keywords}%") if in_content? && keywords.present?
